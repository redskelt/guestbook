...
pipeline {
    ...
    environment {
        ...
        strDockerImage ="stylist40/cicd_guestbook:${strDockerTag}"
    }

    stages {
        stage('Checkout') {
            steps {
                git branch: 'master', url:'https://github.com/redskelt/guestbook.git'

                dir('/root/sub-workspace/guestbook-config'){
                    git branch: 'master', url:'https://github.com/redskelt/guestbook-config.git'
                }
            }
        }
        ...
        stage('Docker Image Push') {
            steps {
                script {
                    docker.withRegistry('', 'DockerHub_stylist40') {
                        oDockImage.push()
                    }
                }
            }
            ...
            steps {
                dir('/root/sub-workspace/guestbook-config'){
                    sh '''
                        ...
                        git push "https://redskelt:${GITHUB_ACCESS_TOKEN}@github.com/redskelt/guestbook-config.git"
                    '''
                }
            }
        }
